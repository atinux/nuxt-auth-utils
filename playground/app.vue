...
<template>
  <UHeader>
    <template #logo> Nuxt Auth Utils </template>
    <template #right>
      <AuthState>
        <UButton
          v-if="!loggedIn"
        ...
        <template #placeholder>
          <UButton size="xs" color="gray" disabled> Loading... </UButton>
        </template>
      </AuthState>
    </template>
  </UHeader>
  ...
  >
    <form @submit.prevent="login">
      <UFormGroup label="Password">
        <UInput v-model="password" name="password" type="password" />
      </UFormGroup>
      <UButton type="submit" :disabled="!password" color="black" class="mt-2">
        Login
      </UButton>
    </form>
  </UDashboardModal>
